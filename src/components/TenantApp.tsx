...
import { 
  ...
  Moon, Sun, Clock, X, CreditCard, ShieldCheck
} from 'lucide-react';
...
export default function TenantApp({ onLogout, toggleDarkMode, isDarkMode }: TenantAppProps) {
    ...
    >
      <div className="flex items-center justify-between mb-2">
        <div>
          ...
          <p className="text-text-sec text-sm">Kamar {currentUser.roomNumber}</p>
        </div>
      ...
      <div className="bg-[#007AFF] rounded-[28px] p-6 text-white shadow-[0_10px_30px_rgba(0,122,255,0.2)] relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <div className="flex items-center justify-between mt-6 pt-6 border-t border-white/20">
            ...
            <button onClick={() => setIsPaymentModalOpen(true)} className="bg-white text-[#007AFF] px-6 py-2 rounded-full text-sm font-bold shadow-sm hover:scale-105 transition-transform">
              Bayar Sekarang
            </button>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <button onClick={() => setIsIssueModalOpen(true)} className="bg-ios-card p-5 rounded-[24px] border border-border-subtle shadow-[0_4px_20px_rgba(0,0,0,0.02)] flex flex-col gap-3 items-start hover:border-border-subtle/80 transition-colors">
          ...
          <div key={payment.id} className="bg-ios-card rounded-[24px] border border-border-subtle shadow-[0_4px_20px_rgba(0,0,0,0.02)] p-5">
            ...
            <div className="pt-4 border-t border-border-subtle flex justify-between items-center">
              ...
              {payment.status === 'paid' ? (
                ...
                <button onClick={() => setIsPaymentModalOpen(true)} className="bg-[#007AFF] text-white px-5 py-2 rounded-full text-sm font-bold shadow-sm hover:opacity-90 transition-colors">
                  Bayar
                </button>
              )}
            </div>
  ...
  return (
    <div className="min-h-screen bg-ios-bg flex overflow-hidden">
        ...
        <div className="pt-6 border-t border-border-subtle">
          <div className="flex items-center gap-3 p-4 bg-bg-subtle rounded-[20px] cursor-pointer hover:bg-bg-hover transition-colors border border-border-subtle">
            ...
            <div className="flex-1 overflow-hidden">
              ...
              <p className="text-xs text-text-sec">Kamar {currentUser.roomNumber}</p>
            </div>
          ...
          {activeTab === 'profile' && (
             <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="max-w-2xl mx-auto w-full py-8 space-y-6">
               ...
               <div className="bg-ios-card rounded-[24px] border border-border-subtle p-6 flex flex-col items-center shadow-[0_4px_20px_rgba(0,0,0,0.02)]">
                  <img src={currentUser.avatar} alt="Profile" className="w-24 h-24 rounded-full object-cover mb-4 border-2 border-border-subtle" />
                  <h2 className="font-heading font-bold text-xl text-text-main">{currentUser.name}</h2>
                  <p className="text-text-sec text-sm mb-6">Kamar {currentUser.roomNumber} • Masuk sejak {currentUser.entryDate}</p>
      ...
      <AnimatePresence>
        {isIssueModalOpen && (
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            ...
            >
              <button onClick={() => setIsIssueModalOpen(false)} className="absolute top-4 right-4 p-2 text-text-sec hover:bg-bg-subtle rounded-full transition-colors">
                <X className="w-5 h-5" />
              </button>
              
              <div className="flex items-center gap-3 mb-6">
                <div className="w-12 h-12 rounded-full bg-[#FF9F0A]/10 flex items-center justify-center">
                  <AlertCircle className="w-6 h-6 text-[#FF9F0A]" />
                </div>
                <div>
                  <h2 className="font-heading font-bold text-xl text-text-main">Lapor Kendala</h2>
                  <p className="text-sm text-text-sec">Tim kami akan segera menindaklanjuti</p>
                </div>
              </div>

              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-text-main mb-2">Jenis Kendala</label>
                  <select 
                    value={issueType}
                    onChange={(e) => setIssueType(e.target.value)}
                    className="w-full bg-bg-subtle border border-border-subtle rounded-[16px] px-4 py-3 text-text-main focus:outline-none focus:border-[#007AFF]"
                  >
                    <option value="AC">AC Bermasalah</option>
                    <option value="Listrik">Listrik / Lampu Mati</option>
                    <option value="Air">Air / Kamar Mandi</option>
                    <option value="WiFi">WiFi Lambat / Mati</option>
                    <option value="Lainnya">Lainnya</option>
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-medium text-text-main mb-2">Deskripsi (Opsional)</label>
                  <textarea 
                    value={issueDesc}
                    onChange={(e) => setIssueDesc(e.target.value)}
                    className="w-full bg-bg-subtle border border-border-subtle rounded-[16px] px-4 py-3 text-text-main focus:outline-none focus:border-[#007AFF] min-h-[100px] resize-none"
                    placeholder="Contoh: AC kurang dingin sejak semalam..."
                  ></textarea>
                </div>
                ...
                <button 
                  ...
                  className="w-full bg-[#007AFF] text-white py-4 rounded-[16px] font-bold mt-4 hover:opacity-90 transition-colors"
                >
                  Kirim Laporan
                </button>
              </div>
      ...
      <AnimatePresence>
        {isPaymentModalOpen && (
          <div className="fixed inset-0 bg-black/50 z-50 flex items-end md:items-center justify-center p-0 md:p-4 pb-0 md:pb-4">
            <motion.div
              initial={{ opacity: 0, y: "100%" }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: "100%" }}
              transition={{ type: "spring", damping: 25, stiffness: 200 }}
              className="bg-ios-card w-full max-w-md md:rounded-[32px] rounded-t-[32px] p-6 shadow-2xl relative max-h-[90vh] overflow-y-auto"
            >
              {!paymentSuccess ? (
                <>
                  ...
                  <div className="text-center mb-8 mt-2">
                    <p className="text-sm text-text-sec mb-1">Total Pembayaran</p>
                    <h2 className="font-heading font-bold text-4xl text-text-main">{formatCurrency(currentPayment?.amount || 1500000)}</h2>
                    <p className="text-xs text-[#FF9F0A] mt-2 font-medium bg-[#FF9F0A]/10 inline-block px-3 py-1 rounded-full">Jatuh Tempo: {currentPayment?.dueDate}</p>
                  </div>

                  <div className="space-y-6">
                    <div>
                      <h3 className="font-semibold text-text-main mb-3 text-sm flex items-center gap-2"><CreditCard className="w-4 h-4" /> Metode Pembayaran</h3>
                      <div className="space-y-3">
                        {['BCA Virtual Account', 'Mandiri Virtual Account', 'QRIS', 'GoPay / OVO'].map(method => (
                          <button 
                            key={method}
                            onClick={() => setSelectedPaymentMethod(method)}
                            className={`w-full flex items-center justify-between p-4 rounded-[20px] border transition-all ${selectedPaymentMethod === method ? 'border-[#007AFF] bg-[#007AFF]/5' : 'border-border-subtle bg-bg-subtle hover:bg-bg-hover'}`}
                          >
                            <span className="font-medium text-text-main text-sm">{method}</span>
                            <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${selectedPaymentMethod === method ? 'border-[#007AFF]' : 'border-border-subtle'}`}>
                               {selectedPaymentMethod === method && <div className="w-2.5 h-2.5 rounded-full bg-[#007AFF]"></div>}
                            </div>
                          </button>
                        ))}
                      </div>
                    ...
                    <button 
                      ...
                      className={`w-full py-4 rounded-[16px] font-bold flex items-center justify-center gap-2 transition-all ${selectedPaymentMethod ? 'bg-[#007AFF] text-white hover:opacity-90 shadow-[0_4px_15px_rgba(0,122,255,0.3)]' : 'bg-bg-subtle text-text-sec cursor-not-allowed'}`}
                    >
                      <ShieldCheck className="w-5 h-5" /> Bayar Aman Sekarang
                    </button>
                    <p className="text-center text-[10px] text-text-sec flex items-center justify-center gap-1">
                      <ShieldCheck className="w-3 h-3" /> Transaksi dilindungi enkripsi SSL 256-bit
                    </p>
                  </div>
                </>
              ) : (
                <div className="flex flex-col items-center justify-center py-12 text-center space-y-4">
                   ...
                   <p className="text-sm text-text-sec">Terima kasih, pembayaran sewa kos Anda telah kami terima.</p>
                </div>
              )}
            </motion.div>
          </div>
